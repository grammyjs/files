...
export async function* readFile(path: URL): AsyncIterable<Uint8Array> {
    const file = await Deno.open(path);
    yield* file.readable;
}
// Copy a file from a URL to a file path
export async function downloadFile(url: URL, dest: string) {
    if (url.protocol === "file") return await copyFile(url, dest);
